...
#define DEFAULT_USART_COMMAND_TIMEOUT_MS		200

TVNAUsartCommsForm *VNAUsartCommsForm = NULL;

__fastcall TVNAUsartCommsForm::TVNAUsartCommsForm(TComponent* Owner)
	: TForm(Owner)
{
}

void __fastcall TVNAUsartCommsForm::FormCreate(TObject *Sender)
{
	AnsiString s;
	...
	m_command_timeout_ms = DEFAULT_USART_COMMAND_TIMEOUT_MS;
	TimeoutEdit->Text = IntToStr(m_command_timeout_ms);

//	s.printf("%d %d", Memo1->Text.Length(), Memo1->Lines->Count);
//	StatusBar1->Panels->Items[0]->Text = s;

	{
		const TNotifyEvent ne = CommandEndsComboBox->OnChange;
		CommandEndsComboBox->OnChange = NULL;
		CommandEndsComboBox->ItemIndex = settings.vnaUSARTCommandEnds;
		CommandEndsComboBox->OnChange = ne;
	}

	// **********************

	{
		VNAUsartBaudrateComboBox->Items->BeginUpdate();

		VNAUsartBaudrateComboBox->Clear();
		VNAUsartBaudrateComboBox->AddItem("1200", (TObject *)1200);
		VNAUsartBaudrateComboBox->AddItem("2400", (TObject *)2400);
		VNAUsartBaudrateComboBox->AddItem("4800", (TObject *)4800);
		VNAUsartBaudrateComboBox->AddItem("9600", (TObject *)9600);
		VNAUsartBaudrateComboBox->AddItem("19200", (TObject *)19200);
		VNAUsartBaudrateComboBox->AddItem("38400", (TObject *)38400);
		VNAUsartBaudrateComboBox->AddItem("57600", (TObject *)57600);
		VNAUsartBaudrateComboBox->AddItem("76800", (TObject *)76800);
		VNAUsartBaudrateComboBox->AddItem("115200", (TObject *)115200);
		VNAUsartBaudrateComboBox->AddItem("230400", (TObject *)230400);
		VNAUsartBaudrateComboBox->AddItem("250000", (TObject *)250000);
		VNAUsartBaudrateComboBox->AddItem("460800", (TObject *)460800);
		VNAUsartBaudrateComboBox->AddItem("500000", (TObject *)500000);
		VNAUsartBaudrateComboBox->AddItem("921600", (TObject *)921600);
		VNAUsartBaudrateComboBox->AddItem("1000000", (TObject *)1000000);
		VNAUsartBaudrateComboBox->AddItem("1843200", (TObject *)1843200);
		VNAUsartBaudrateComboBox->AddItem("2000000", (TObject *)2000000);
		VNAUsartBaudrateComboBox->AddItem("3000000", (TObject *)3000000);
		VNAUsartBaudrateComboBox->AddItem("3686400", (TObject *)3686400);

		const int i = VNAUsartBaudrateComboBox->Items->IndexOfObject((TObject *)115200);
		if (i >= 0)
			VNAUsartBaudrateComboBox->ItemIndex = i;
		else
			VNAUsartBaudrateComboBox->ItemIndex = 8;

		VNAUsartBaudrateComboBox->Items->EndUpdate();

		common.comboBoxAutoWidth(VNAUsartBaudrateComboBox);
	}

	// **********************

	// move to the saved position
	this->Top    = settings.vnaUSARTcommsWindowPos.top;
	this->Left   = settings.vnaUSARTcommsWindowPos.left;
	this->Width  = settings.vnaUSARTcommsWindowPos.width;
	this->Height = settings.vnaUSARTcommsWindowPos.height;
}

void __fastcall TVNAUsartCommsForm::FormDestroy(TObject *Sender)
{
	//
}

void __fastcall TVNAUsartCommsForm::WMWindowPosChanging(TWMWindowPosChanging &msg)
{
...
void __fastcall TVNAUsartCommsForm::CloseButtonClick(TObject *Sender)
{
	if (!Memo1->ReadOnly)
...
void __fastcall TVNAUsartCommsForm::SaveButtonClick(TObject *Sender)
{
	if (!Memo1->ReadOnly)
	{  // being used to edit user commands
...
void __fastcall TVNAUsartCommsForm::ClearButtonClick(TObject *Sender)
{
	clear();
}

void __fastcall TVNAUsartCommsForm::FormShow(TObject *Sender)
{
	if (!m_initialised)
	{
		// move to the saved position
...
void __fastcall TVNAUsartCommsForm::FormHide(TObject *Sender)
{
	saveCommandHistory();
	saveUserCommands();
}

void __fastcall TVNAUsartCommsForm::FormClose(TObject *Sender, TCloseAction &Action)
...
void __fastcall TVNAUsartCommsForm::loadCommandHistory()
{
	const String filename = ExtractFilePath(Application->ExeName) + VNA_USART_COMMS_COMMAND_HISTORY_FILENAME;
...
void __fastcall TVNAUsartCommsForm::saveCommandHistory()
{
	const String filename = ExtractFilePath(Application->ExeName) + VNA_USART_COMMS_COMMAND_HISTORY_FILENAME;
	common.saveFile(filename, m_command_history);
}

void __fastcall TVNAUsartCommsForm::loadUserCommands()
{
	...
	{
		String filename;
		filename.printf(L"vna_usart_user_command_%u.txt", i);
...
void __fastcall TVNAUsartCommsForm::saveUserCommands()
{
	const String path = ExtractFilePath(Application->ExeName);
...
void __fastcall TVNAUsartCommsForm::show()
{
	update();
...
void __fastcall TVNAUsartCommsForm::clear()
{
	Memo1->Clear();
...
void __fastcall TVNAUsartCommsForm::addText(AnsiString prefix, AnsiString s)
{
	AnsiString s2;
...
//	s2.printf("%d %d", Memo1->Text.Length(), Memo1->Lines->Count);
//	StatusBar1->Panels->Items[0]->Text = s2;

	SaveButton->Enabled = !Memo1->Text.IsEmpty();
}

void __fastcall TVNAUsartCommsForm::setBaudrate(int baudrate)
{
	const int i = VNAUsartBaudrateComboBox->Items->IndexOfObject((TObject *)baudrate);
	if (i >= 0)
	{
		const TNotifyEvent ne = VNAUsartBaudrateComboBox->OnChange;
		VNAUsartBaudrateComboBox->OnChange = NULL;
		VNAUsartBaudrateComboBox->ItemIndex = i;
		VNAUsartBaudrateComboBox->OnChange = ne;
	}
}

void __fastcall TVNAUsartCommsForm::update()
{
	SaveButton->Enabled = !Memo1->Text.IsEmpty();
}

void __fastcall TVNAUsartCommsForm::CommandEditKeyDown(TObject *Sender, WORD &Key,
			 TShiftState Shift)
{
	switch (Key)
	{
		case VK_RETURN:
			Key = 0;
			{
				...
				{
					String s2 = "\"" + s + "\"";
					String s3 = (m_command_timeout_ms != DEFAULT_USART_COMMAND_TIMEOUT_MS) ? " " + IntToStr(m_command_timeout_ms) : String("");
					addText(FormatDateTime("ss.zzz ", Now()) + "tx: ", s2 + s3);
					Form1->addSerialTxCommand("usart " + s2  + s3, settings.vnaUSARTCommandEnds);
				}
...
void __fastcall TVNAUsartCommsForm::CommandEditEnter(TObject *Sender)
{
	if (Form1)
	{
//		const String s = CommandEdit->Text.Trim();
...
void __fastcall TVNAUsartCommsForm::CommandEditExit(TObject *Sender)
{
	if (Form1)
	{
//		const String s = CommandEdit->Text.Trim();
...
void __fastcall TVNAUsartCommsForm::CommandEditChange(TObject *Sender)
{
	if (Form1)
	{
//		const String s = CommandEdit->Text.Trim();
...
void __fastcall TVNAUsartCommsForm::VNAUsartBaudrateComboBoxChange(TObject *Sender)

{
	int i = VNAUsartBaudrateComboBox->ItemIndex;
	if (i >= 0)
	{
		const int baudrate = (int)VNAUsartBaudrateComboBox->Items->Objects[i];

		if (Form1)
		{
			String s;
			s.printf(L"usart_cfg %d", baudrate);
			Form1->addSerialTxCommand(s, settings.vnaUSARTCommandEnds);
		}
	}
}

void __fastcall TVNAUsartCommsForm::CommandsButtonPopupMenuPopup(TObject *Sender)

{
	Paste1->Enabled = Clipboard()->HasFormat(CF_TEXT);
}

TButton * __fastcall TVNAUsartCommsForm::getPopupMenuButton()
{
	TButton *but = NULL;
...
void __fastcall TVNAUsartCommsForm::editUserCommand(const int index)
{
...
void __fastcall TVNAUsartCommsForm::Clearcommand1Click(TObject *Sender)
{
...
void __fastcall TVNAUsartCommsForm::Editcommand1Click(TObject *Sender)
{
	TButton *but = getPopupMenuButton();
	if (but == CommandsButton1)
	{
...
void __fastcall TVNAUsartCommsForm::Copy1Click(TObject *Sender)
{
	TButton *but = getPopupMenuButton();
	int index = -1;
...
void __fastcall TVNAUsartCommsForm::Paste1Click(TObject *Sender)
{
	TButton *but = getPopupMenuButton();
	if (but == NULL)
		return;
...
void __fastcall TVNAUsartCommsForm::Rename1Click(TObject *Sender)
{
	TButton *but = getPopupMenuButton();
	if (but == NULL)
		return;
...
void __fastcall TVNAUsartCommsForm::Cancel1Click(TObject *Sender)
{
	m_popup_menu_button = NULL;
}

void __fastcall TVNAUsartCommsForm::CommandsButtonClick(TObject *Sender)
{
	...
	{	// send the commands
		...
		{
			String s1 = "\"" + cmds[i] + "\"";
			String s2 = (m_command_timeout_ms != DEFAULT_USART_COMMAND_TIMEOUT_MS) ? " " + IntToStr(m_command_timeout_ms) : String("");
			addText(FormatDateTime("ss.zzz ", Now()) + "tx: ", s1 + s2);
			Form1->addSerialTxCommand("usart " + s1 + s2, settings.vnaUSARTCommandEnds);
		}
	}
}

void __fastcall TVNAUsartCommsForm::CommandsButtonDropDownClick(TObject *Sender)
...
void __fastcall TVNAUsartCommsForm::CommandEndsComboBoxClick(TObject *Sender)

{
	settings.vnaUSARTCommandEnds = CommandEndsComboBox->ItemIndex;
}

void __fastcall TVNAUsartCommsForm::FormKeyDown(TObject *Sender, WORD &Key, TShiftState Shift)
...
void __fastcall TVNAUsartCommsForm::TimeoutEditChange(TObject *Sender)
{
	int i;
	if (TryStrToInt(TimeoutEdit->Text, i))
	{
		if (i < 0)
			i = 0;
		m_command_timeout_ms = i;
	}
}

void __fastcall TVNAUsartCommsForm::FormMouseEnter(TObject *Sender)
{
//	this->SetFocus();
}
