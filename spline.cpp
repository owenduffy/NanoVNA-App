...
#include <System.Math.hpp>	// IsNan() IsInfinite()
...
MonotCubicInterpolator::MonotCubicInterpolator(const std::vector <double> &x, const std::vector <double> &f)
{
	if (x.size() != f.size())
		throw("Unable to constuct MonotCubicInterpolator from vectors.");

	// Add the contents of the input vectors to our map of values.
	std::vector <double>::const_iterator posx;
	std::vector <double>::const_iterator posf;
...
/*
bool __fastcall MonotCubicInterpolator::read(const std::string &datafilename, int xColumn, int fColumn)
{
	data.clear();
	ddata.clear();

	std::ifstream datafile_fs(datafilename.c_str());
	if (!datafile_fs)
		return false;
	...
	{
		std::getline(datafile_fs, linestring);

		// Replace commas with space:
		std::string::size_type pos = 0;
		while ((pos = linestring.find(",", pos)) != std::string::npos)
		{
		...
		std::stringstream strs(linestring);
		int columnindex = 0;
...
*/
void __fastcall MonotCubicInterpolator::addPair(double newx, double newf, bool compute) throw(const char *)
{
...
double __fastcall MonotCubicInterpolator::evaluate(double x) const throw(const char *)
{
	if (IsNan(x) || IsInfinite(x))
		throw("MonotCubicInterpolator: evaluate() received inf/nan input.");

	// xf becomes the first (xdata, fdata) pair where xdata >= x
	std::map <double, double>::const_iterator xf_iterator = data.lower_bound(x);
	...
	std::pair <double, double> xf2 = *xf_iterator;
	std::pair <double, double> xf1 = *(--xf_iterator);
	...
	{
		double finterp = xf1.second + (xf2.second - xf1.second) / (xf2.first - xf1.first) * (x - xf1.first);
		return finterp;
	}
	...
	double finterp = (xf1.second * H00(t)) + (ddata[xf1.first] * H10(t) * h) + (xf2.second * H01(t)) + (ddata[xf2.first] * H11(t) * h);
	return finterp;
}

// double __fastcall MonotCubicInterpolator::evaluate(double x, double& errorestimate_output)
// {
...
std::vector <double> __fastcall MonotCubicInterpolator::get_xVector() const
{
	std::map <double, double>::const_iterator xf_iterator = data.begin();
...
std::vector <double> __fastcall MonotCubicInterpolator::get_fVector() const
{
	std::map <double, double>::const_iterator xf_iterator = data.begin();
...
std::string __fastcall MonotCubicInterpolator::toString() const
{
	...
	{
		dataStringStream << std::setprecision(precision) << it->first;
		dataStringStream << '\t';
		dataStringStream << std::setprecision(precision) << it->second;
		dataStringStream << '\n';
	}
	dataStringStream << "Derivative values:" << std::endl;
	for (std::map <double, double>::const_iterator it = ddata.begin(); it != ddata.end(); ++it)
	{
		dataStringStream << std::setprecision(precision) << it->first;
		dataStringStream << '\t';
		dataStringStream << std::setprecision(precision) << it->second;
...
std::pair <double, double> __fastcall MonotCubicInterpolator::getMissingX() const throw(const char *)
{
	if (data.size() < 2)
		throw("__fastcall MonotCubicInterpolator::getMissingX() only one datapoint.");

	// Search for biggest difference value in function-datavalues:

	std::map <double, double>::const_iterator maxfDiffPair_iterator = data.begin();;
	double maxfDiffValue = 0.0;

	std::map <double, double>::const_iterator xf_iterator;
	std::map <double, double>::const_iterator xf_next_iterator;

	for (xf_iterator = data.begin(), xf_next_iterator = ++(data.begin()); xf_next_iterator != data.end(); ++xf_iterator, ++xf_next_iterator)
	{
		double absfDiff = fabs((double)(*xf_next_iterator).second - (double)(*xf_iterator).second);
   ...
   return std::make_pair(newXvalue, maxfDiffValue);
}

std::pair <double, double> __fastcall MonotCubicInterpolator::getMaximumF() const throw(const char*)
{
	if (data.size() <= 1)
		throw ("__fastcall MonotCubicInterpolator::getMaximumF() empty data.");
	...
	std::pair <double, double> maxf = *data.rbegin();
   std::map <double, double> :: const_iterator xf_iterator;
   for (xf_iterator = data.begin(); xf_iterator != data.end(); ++xf_iterator)
   {
...
std::pair <double, double> __fastcall MonotCubicInterpolator::getMinimumF() const throw(const char*)
{
   if (data.size() <= 1)
      throw ("__fastcall MonotCubicInterpolator::getMinimumF() empty data.");
   ...
   std::pair <double, double> minf = *data.rbegin();
   std::map <double, double> :: const_iterator xf_iterator;
   for (xf_iterator = data.begin(); xf_iterator != data.end(); ++xf_iterator)
   {
...
void __fastcall MonotCubicInterpolator::computeInternalFunctionData() const
{
	// The contents of this function is meaningless if there is only one datapoint
	if (data.size() <= 1)
		return;

	// We do not check the caching flag if we are instructed to do this computation

	// We compute monotoneness and directions by assuming monotoneness, and setting to false if the function is not for some value
	...
	while (xf_next_iterator != data.end() && xf_iterator->second == xf_next_iterator->second)
	{
...
void __fastcall MonotCubicInterpolator::chopFlatEndpoints(const double epsilon)
{
   if (getSize() < 3)
      return;

   std::map <double, double>::iterator xf_iterator;
   std::map <double, double>::iterator xf_next_iterator;
   ...
   while ((xf_next_iterator != data.end()) && (fabs(xf_iterator->second - xf_next_iterator->second) < epsilon))
   {
      xf_next_iterator++;
   ...
   while ((xf_next_iterator != data.begin()) && (fabs(xf_iterator->second - xf_next_iterator->second) < epsilon))
   {
      xf_next_iterator--;
      data.erase(xf_iterator);
      xf_iterator--;
...
//
//
void __fastcall MonotCubicInterpolator::shrinkFlatAreas(const double epsilon)
{
   if (getSize() < 2)
      return;
   ...
   // Clear flags, they are not to be trusted after we modify the data
   strictlyMonotoneCached = false;
	monotoneCached = false;
   ...
   while (xf_next_iterator != data.end())
   {
      ...
      if (fabs(xf_iterator->second - xf_next_iterator->second) < epsilon)
      {
         ...
         std::map <double,double>::iterator xf_tobedeleted_iterator = xf_next_iterator;
         xf_next_iterator++;
...
void __fastcall MonotCubicInterpolator::computeSimpleDerivatives() const
{
	ddata.clear();

	// Do endpoints first:
   std::map <double, double>::const_iterator xf_prev_iterator;
   std::map <double, double>::const_iterator xf_iterator;
   std::map <double, double>::const_iterator xf_next_iterator;
   double diff;
   ...
   // Leftmost interval:
	xf_iterator = data.begin();
   xf_next_iterator = ++(data.begin());
   diff = (xf_next_iterator->second - xf_iterator->second) / (xf_next_iterator->first - xf_iterator->first);
   ...
   if (data.size() > 2)
   {
      std::map <double, double>::const_iterator intpoint;
      for (intpoint = ++data.begin(); intpoint != --data.end(); ++intpoint)
      {
         ...
         std::map <double, double> :: const_iterator lastpoint = intpoint; --lastpoint;
         std::map <double, double> :: const_iterator nextpoint = intpoint; ++nextpoint;

         diff = (nextpoint->second - intpoint->second) / (2 * (nextpoint->first - intpoint->first)) + (intpoint->second - lastpoint->second) / ((2 * (intpoint->first) - lastpoint->first));
...
void __fastcall MonotCubicInterpolator::adjustDerivativesForMonotoneness() const
{
   std::map <double, double> :: const_iterator point;
   std::map <double, double> :: const_iterator dpoint;

   // Loop over all intervals, ie. loop over all points and look at the interval to the right of the point
	for (point = data.begin(), dpoint = ddata.begin(); point != --data.end(); ++point, ++dpoint)
   {
      std::map <double, double> :: const_iterator nextpoint;
      std::map <double, double> :: const_iterator nextdpoint;
      nextpoint = point;
      ++nextpoint;
      nextdpoint = dpoint;
      ++nextdpoint;

      double delta = (nextpoint->second - point->second) / (nextpoint->first - point->first);
      if (fabs(delta) < 1e-14)
		{
...
void __fastcall MonotCubicInterpolator::scaleData(double factor)
{
	std::map <double, double>::iterator it;
	std::map <double, double>::iterator itd;
